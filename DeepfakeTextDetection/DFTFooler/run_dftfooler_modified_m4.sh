...
ORIGINAL_DATA_DIR="../M4_data_processed"
ADVERSARIAL_DATA_DIR="./adversarial_datasets"
CSV_DIR="./adversarial_stats_csv" # Assuming both csv1 and csv2 are in the same directory

# Arrays for dataset and model types
DATASET_TYPES=("arxiv" "peerread" "reddit" "wikihow" "wikipedia")
MODEL_TYPES=("bloomz" "chatgpt" "cohere" "davinci" "dolly")

# Nested loop to process all combinations
for dataset in "${DATASET_TYPES[@]}"; do
    for model in "${MODEL_TYPES[@]}"; do
        ATTACK_DATASET="${ORIGINAL_DATA_DIR}/processed_${dataset}_${model}.jsonl"
        OUTPUT_TEXT="${ADVERSARIAL_DATA_DIR}/adversarial_processed_${dataset}_${model}.jsonl"

...
from contrastive_utils.models import SimpleNNRoBERTa
...
def test_model(checkpoint_path: str,
    ...
    model = SimpleNNRoBERTa(num_classes=num_classes)
    ...
    if num_classes == 2 or include_filters[0] != '':
        ...
        target_names = ["Human", "Bloomz",
                        "ChatGPT", "Cohere", "Davinci", "Dolly"]
...
def test_model_inthewild(checkpoint_path: str,
    ...
    results_save_name = checkpoint_path.split('/')[1] + f'_{keyword}'
    model = SimpleNNRoBERTa(num_classes=num_classes)
    checkpoint = torch.load(checkpoint_path, map_location=device)
    ...
    dataset = InTheWild(root_dir, keyword=keyword)
    dataloader = DataLoader(dataset, batch_size=32, shuffle=False)
...
# base_dir = 'classification_results_roberta/'
# model_dir = 'results_0_roberta/'
# embedding_data_path = '../data/embedding_data_roberta_m4_npy/'
...
experiments = [
    ('allm4_2class', 2, 'ai_writer'),
    ('allm4_6class', 6, 'ai_writer'),
    ('allm4_2class', 2, 'ArticleForge'),
    ('allm4_6class', 6, 'ArticleForge'),
    ('allm4_2class', 2, 'kafkai'),
    ('allm4_6class', 6, 'kafkai'),
    ('allm4_2class', 2, 'reddit'),
    ('allm4_6class', 6, 'reddit'),
]

base_dir = 'classification_results_roberta_inthewild/'
model_dir = 'results_0_roberta/'
embedding_data_path = '../data/embedding_data_roberta_inthewild_npy/'

for model_name, num_classes, keyword in experiments:
    model_path = model_dir + model_name + '/contrastive_model_best.pt'
    test_model_inthewild(model_path, embedding_data_path,
                         num_classes=num_classes, save_dir=base_dir, keyword=keyword)
...
# base_dir = 'classification_results_roberta_adversarial/'
# model_dir = 'results_0_roberta/'
# embedding_data_path = '../data/embedding_data_roberta_adversarial_npy/'

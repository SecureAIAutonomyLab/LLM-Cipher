...
for model_filter in model_filters:
    ...
    # source_directory = '../data/embedding_data_T5_json/'
    source_directory = '../data/embedding_data_T5_json/'
    target_directory = f'../data/embedding_data_contrastive_{model_filter}_json/'
    npy_directory = f'../data/embedding_data_contrastive_{model_filter}_npy/'
    ...
    for json_file in glob.glob(os.path.join(source_directory, '*.json')):
        ...
        with open(os.path.join(target_directory, f'{model_name}_{domain_name}_contrastive.json'), 'w') as f:
            json.dump(updated_data_list, f, indent=4)

        np.save(os.path.join(npy_directory,
                f'{model_name}_{domain_name}_human_contrastive.npy'), np.array(new_human_embeddings))
        np.save(os.path.join(npy_directory, f'{model_name}_{domain_name}_machine_contrastive.npy'), np.array(
            new_machine_embeddings))

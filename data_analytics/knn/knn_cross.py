...
results_file = f"knn_results_held_out_{held_out}.txt"
...
for file in os.listdir(folder_name):
    ...
    if held_out in file:
        ...
        print(f"Adding file '{file}' to Test Data with class '{class_name}' and label {label}")
    else:
        train_data.extend(data)
        train_labels.extend([label]*len(data))
        print(f"Adding file '{file}' to Train Data with class '{class_name}' and label {label}")

train_data, test_data = np.array(train_data), np.array(test_data)
train_labels, test_labels = np.array(train_labels), np.array(test_labels)

print(f"Total size of Train Data: {len(train_data)}")

...
folder_name = "../../data/embedding_data_contrastive_crossdomain_chatgpt_npy/"
...
for train_dataset in data_set_mapping.values():
    for cl_name in data_set_mapping.values():
        X_train_full, y_train_full = load_and_preprocess_data(constant_generator, train_dataset, perturbation, cl_name)
        ...
        for test_dataset in data_set_mapping.values():
            X_test_full, y_test_full = load_and_preprocess_data(constant_generator, test_dataset, perturbation, cl_name)
            ...
            for split_name, X_split, y_split in zip(['Test', 'Validation'], [X_test, X_val], [y_test, y_val]):
                ...
                results_str = f"Train on {train_dataset}, {split_name} on {test_dataset} with CL name {cl_name}\n"
                ...
                result_file_name = f"cross_domain_knn_{constant_generator}_{n_neighbors}_{metric}_{perturbation}_{cl_name}.txt"
                write_statistics_to_file(os.path.join(results_folder, result_file_name), results_str)
                
                print(f"Trained on: {train_dataset}. Evaluated on: {test_dataset}. Split: {split_name}. Constant Generator: {constant_generator}. Number of Neighbors: {n_neighbors}. Metric: {metric}. Perturbation: {perturbation}. CL name: {cl_name}.")

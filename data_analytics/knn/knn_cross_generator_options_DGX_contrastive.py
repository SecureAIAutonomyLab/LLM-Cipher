...
parser.add_argument('--dataset', choices=data_set_mapping.values(), required=True, help='The dataset to use')
parser.add_argument('--perturbation', type=str, default='original', help='Type of perturbation')
args = parser.parse_args()

n_neighbors = args.n_neighbors
metric = args.metric
constant_dataset = args.dataset
...
for train_generator in generator_mapping.values():
    X_train_full, y_train_full = load_and_preprocess_data(train_generator, constant_dataset, perturbation)
    ...
    for test_generator in generator_mapping.values():
        X_test_full, y_test_full = load_and_preprocess_data(test_generator, constant_dataset, perturbation)
        ...
        for split_name, X_split, y_split in zip(['Test', 'Validation'], [X_test, X_val], [y_test, y_val]):
            ...
            results_str = f"Train with {train_generator}, {split_name} on {test_generator}\n"
            ...
            result_file_name = f"cross_generator_knn_{constant_dataset}_{n_neighbors}_{metric}_{perturbation}.txt"
            write_statistics_to_file(os.path.join(results_folder, result_file_name), results_str)
            
            print(f"Trained on: {train_generator}. Evaluated on: {test_generator}. Split: {split_name}. Constant Dataset: {constant_dataset}. Number of Neighbors: {n_neighbors}. Metric: {metric}. Perturbation: {perturbation}.")

...
folder_name = "embedding_data"
results_folder = "gmm_results"


def train_and_evaluate_gmm(X_train, X_test, y_train):
    # Since the labels are binary (0 or 1), we'll use 2 components for GMM
...
# Ensure the 'gmm_results' directory exists
if not os.path.exists(results_folder):
    os.makedirs(results_folder)


results_filename = os.path.join(results_folder, "gmm_individual_results.txt")
...
for data_set_name in data_set_mapping.values():
    for generator_name in generator_mapping.values():
        ...
        if os.path.exists(file_path):  
            ...
            # Evaluating on test data using GMM
            test_predictions = train_and_evaluate_gmm(X_train, X_test, y_train)

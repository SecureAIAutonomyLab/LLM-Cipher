...
for excluded_dataset in data_set_mapping.values():
    processed_human_embeddings.clear()  # Clear for each dataset
    X_train, y_train = [], []
    for data_set in data_set_mapping.values():
        if data_set != excluded_dataset:
    ...
    X_test, y_test = [], []
    for gen in generator_mapping.values():
        embeddings, labels = load_data_from_file(os.path.join(folder_name, f"{excluded_dataset}_{gen}_emb.jsonl"))
    ...
    report_str = f"Results when excluding {excluded_dataset}:\n"
    ...
    results_filename = os.path.join(results_folder, f"knn_exclude_{excluded_dataset}_results.txt")
    write_statistics_to_file(results_filename, report_str)
    print(f"Finished processing excluding {excluded_dataset}")
